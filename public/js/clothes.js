// Esperar a que todo el HTML se cargue antes de ejecutar el JS
document.addEventListener('DOMContentLoaded', () => {

  // ==========================================
  // 1. PRIMER GRUPO: ACTIVE ORDERS (3 por fila)
  // ==========================================
  const orders = [
    {
      status: "In Progress",
  ...
  function renderOrders(data) {
    ...
    if (!ordersContainer) return; // Si no existe el contenedor en la página, evita errores
      ...
      <div class="col-12 col-md-6 col-lg-4">
        <div class="card h-100 border-0 shadow-sm rounded-4 overflow-hidden bg-white">
          <div class="p-3 d-flex align-items-center justify-content-between border-bottom bg-light-subtle">
            ${order.statusBg 
              ? `<span class="badge rounded-pill px-3 py-2 fw-semibold" style="background-color: ${order.statusBg}; color: ${order.statusColor};">${order.status}</span>`
              : `<span class="badge ${order.statusClass} rounded-pill px-3 py-2 fw-semibold">${order.status}</span>`}
            <small class="text-muted fw-semibold" style="font-size: 0.8rem;">${order.date}</small>
          </div>
          <div class="card-body p-3 d-flex flex-column">
            ...
            <div class="mt-auto pt-2 d-flex gap-2">
              <button class="btn btn-outline-dark btn-sm flex-fill rounded-3 fw-semibold px-1" style="font-size: 0.78rem;">
                <i class="bi bi-chat-dots me-1"></i> Chat
              </button>
              <button class="btn btn-sm flex-fill rounded-3 fw-semibold text-white px-1" style="background-color: #1b4332; font-size: 0.78rem;">
                Update Status
              </button>
            </div>
          </div>
  ...
  // ==========================================
  // 2. SEGUNDO GRUPO: CUSTOMIZATION REQUESTS (4 por fila)
  // ==========================================
  const requestsData = [
    {
  ...
  function renderCustomizationCards(requests) {
    ...
    if (!container) return; // Si no existe el contenedor, evita que falle la ejecución

    container.innerHTML = requests.map(item => `
      <div class="col-12 col-md-6 col-lg-3 col-xl-3">
        <div class="card h-100 border-0 shadow-sm rounded-4 overflow-hidden bg-white">
          <div class="position-relative" style="height: 190px;">
            <img src="${item.image}" class="w-100 h-100 object-fit-cover" alt="Request design">
          ...
          <div class="card-body p-3 d-flex flex-column">
            ...
            <div class="mt-auto pt-2 border-top d-flex justify-content-between align-items-center">
              ...
              <button class="btn btn-sm px-3 py-2 rounded-3 fw-semibold text-white border-0" style="background-color: #1b4332; font-size: 0.8rem;">
                Apply / Review
              </button>
            </div>
          </div>
